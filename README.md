...
P0-026 smoke test 已完成，当前进入 P0.5 收尾阶段：用户中心、登录注册方案、个人资料和 UI 体验整理。P0.5 不新增 P1 行程、投票、账本、AA、照片墙功能。
...
- 后端新增 `PUT /api/users/me`，只允许修改当前用户自己的昵称、头像 URL 和联系电话。
- 文件上传 `POST /api/files/upload` 扩展支持 `fileType=USER_AVATAR`。
- 小程序新增 `pages/profile-edit/index`，支持上传头像、编辑昵称和联系电话。
- 手机号暂时只作为“联系电话 / 个人资料字段”，不作为登录凭证，不做短信验证码。
