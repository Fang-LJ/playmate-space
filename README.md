...
P1 行程 × 投票协作闭环、字段级结果应用和动态待办中心已经具备。第一版关联已有行程只开放 `FULL_PLAN` 完整方案投票。P2 费用与 AA 结算已开始实现：账单、均摊/自定义分摊、作废、动态 AA 建议、实际线下转账记录与 MinIO 付款凭证均使用同一套活动成员权限。
...
- 继续验证完整方案投票的创建、投票、结果预览和人工确认体验。
- 保留历史快速投票的查询和应用兼容，不在第一版新增快速投票入口。
- 本轮不提前开发费用、照片或其他模块。
...
费用以有效账单和分摊明细为事实来源，AA 建议每次实时计算；只有用户标记完成的线下转账会写入 `t_activity_settlement`。账单作废不会删除历史记录。可执行：
