...
P1 行程 × 投票协作闭环、字段级结果应用和动态待办中心已经具备。六种行程类型统一校验、类型切换清理旧字段、住宿支持跨午夜，行程和投票页面已完成本轮重构。第一版关联已有行程时只开放 `FULL_PLAN` 完整方案投票：标题、时间和具体安排可以随胜出方案更新，行程类型固定不变；历史快速投票继续兼容，新的快速投票入口留到第二版。费用管理与照片墙仍未开发。
