...
P1 UI 收口、行程 × 投票协作闭环和动态待办中心已完成。待办已改为业务事件驱动的持久化生命周期：业务表仍是事实来源，待办表保存任务与成员处理状态；费用管理与照片墙仍未开发。
...
脚本会验证成员创建行程、待决定行程与投票的事务创建、唯一胜出自动应用、并列人工确认、成员权限、协作摘要及结束活动后的只读规则。详细 API 与状态规则见：
