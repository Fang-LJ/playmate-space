...
P1 行程 × 投票协作闭环、字段级结果应用和动态待办中心已经具备。第一版关联已有行程只开放 `FULL_PLAN` 完整方案投票。P2 第一版费用与 AA 已完成并冻结：支持单付款人账单、均摊/自定义金额/按比例分摊、账单作废、付款凭证和实时 AA 建议；第一版不追踪线下转账完成状态。P3 已进入照片墙 Round 1：当前冻结数据模型、内容审核与私有文件架构，业务接口和小程序 UI 留在后续轮次。
...
- `t_user` 是平台用户账号。
- `t_user_identity` 保存微信小程序授权身份。
- 微信登录通过 `t_user_identity` 绑定平台用户，不再依赖 `t_user.openid` 查询用户。
- 支持手机号 / 邮箱 + 密码注册登录。
- 密码使用 BCrypt 哈希存储。
- 微信登录、手机号/邮箱注册成功后都会优先进入原目标页面，不强制跳账号保护页。
- 账号保护页用于主动设置手机号或邮箱及密码；账号保护与头像昵称资料完整度分开计算。
- 本地模拟微信身份固定为 A/B/C，选择同一身份会回到同一个平台账号。
...
- P3 Round 2：实现 `PHOTO` 私有 TEMP 文件、真实性校验、照片绑定/删除/列表、点赞、举报、审核持久任务与短时效访问 URL。
- 费用账单列表后续增加分页和触底加载；真实微信转账及转账状态继续留在后续版本。

## P1 行程与投票联调

除活动详情内的摘要外，当前用户可通过 `GET /api/users/me/activity-todos` 查看按活动聚合的持久化待办。投票创建、投票、关闭、取消、结果应用、成员加入和活动取消会在同一业务事务中更新待办；当前/下一行程只在协作摘要中展示，不计入待办数。

后端启动后执行：

```bash
bash scripts/p1-itinerary-poll-smoke-test.sh
bash scripts/activity-todo-smoke-test.sh
```

脚本会验证结构化交通/用餐行程、普通及历史快速投票兼容、完整方案字段白名单、唯一胜出进入人工确认、标题和执行字段应用、保持当前方案、并列结果预览、应用历史、成员权限、待办生命周期及结束活动后的只读规则。已有数据库需先执行：

```bash
docker exec -i playmate-mysql mysql -u playmate -p playmate_space \
  < docs/sql/p1_003_itinerary_poll_field_linkage.sql
```

迁移脚本可重复执行，不删除已有活动、行程或投票数据。详细 API 与状态规则见：

```text
docs/p1-itinerary-poll-api-design.md
```

历史投票待办可在完成 SQL 迁移后一次性回填。默认不会在每次启动时扫描：仅在需要执行一次时设置 `PLAYMATE_TODO_BACKFILL_ON_STARTUP=true` 启动后端。回填使用幂等 `activity_id + source_key` 和 `todo_id + user_id` 约束，不会删除已有数据。

行程类型业务元数据可通过 `GET /api/itineraries/type-metadata` 获取。接口返回六种类型的重点字段、通用字段和允许决策类型，数据来自后端统一策略；小程序创建完整方案时会据此动态生成当前行程类型的候选方案字段。

## P2 费用与 AA 结算

先对已有开发库执行前向迁移（可重复执行，不删除数据）：
...
P3 Round 1 已冻结照片墙表结构、审核状态机、私有文件生命周期和未来 API 契约，但尚未开放照片业务接口或小程序 UI。已有本地开发库可安全、重复执行：
