...
P1 行程 × 投票协作闭环、字段级结果应用和动态待办中心已经具备。阶段 1 的行程类型策略与后端模型对齐已完成：六种类型统一校验、类型切换清理旧字段、住宿支持跨午夜、关联投票范围由后端按类型推导，并兼容历史 `routeDetail` 和标题投票。阶段 2 的行程页面 Figma 重构和阶段 3 的投票页面重构尚未执行；费用管理与照片墙仍未开发。
...
脚本会验证结构化交通/用餐行程、投票字段白名单、唯一胜出自动应用、并列结果预览、应用历史、成员权限、待办生命周期及结束活动后的只读规则。已有数据库需先执行：
...
行程类型业务元数据可通过 `GET /api/itineraries/type-metadata` 获取。接口返回六种类型的重点字段、通用字段和允许决策类型，数据来自后端统一策略；当前小程序尚未在阶段 2/3 中改为动态消费该接口。
