...
P1 UI 收口、行程 × 投票协作闭环和动态待办中心已完成。首页角标和“我的待办”基于现有行程、投票实时聚合；费用管理与照片墙仍未开发。
...
除活动详情内的摘要外，当前用户可通过 `GET /api/users/me/activity-todos` 查看按活动聚合的动态待办。该接口不新增待办表，待办会随投票参与、投票结束和行程状态实时变化。
