...
public class CollaborationSummaryService {
    ...
    public CollaborationSummaryResponse get(Long activityId){Long userId=access.requireUserId();ActivityEntity activity=access.requireActivity(activityId);access.requireActiveMember(activityId,userId);List<ActivityItineraryEntity> itineraries=itineraryService.sorted(activityId);List<PollListItemResponse>polls=pollService.list(activityId);List<CollaborationTodoResponse>todos=new ArrayList<>();LocalDateTime now=LocalDateTime.now();for(PollListItemResponse p:polls){if("ACTIVE".equals(p.status())&&!Boolean.TRUE.equals(p.currentUserVoted()))todos.add(new CollaborationTodoResponse("POLL_PENDING","参与投票："+p.title(),"POLL",p.pollId()));else if("REVIEW_REQUIRED".equals(p.resultApplyStatus()))todos.add(new CollaborationTodoResponse("POLL_REVIEW","投票结果待确认："+p.title(),"POLL",p.pollId()));if(todos.size()>=3)break;}for(ActivityItineraryEntity i:itineraries){String s=ItineraryTimeStatusResolver.resolve(i,now);if("IN_PROGRESS".equals(s)){todos.add(new CollaborationTodoResponse("ITINERARY_IN_PROGRESS","当前行程："+i.getTitle(),"ITINERARY",i.getId()));break;}if("UPCOMING".equals(s)&&i.getItineraryDate().isBefore(LocalDate.now().plusDays(2))){todos.add(new CollaborationTodoResponse("ITINERARY_SOON","即将开始："+i.getTitle(),"ITINERARY",i.getId()));break;}}
