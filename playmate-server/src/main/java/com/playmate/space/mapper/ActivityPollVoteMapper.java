...
public interface ActivityPollVoteMapper extends BaseMapper<ActivityPollVoteEntity> {
 ...
 @Update("UPDATE t_activity_poll_vote SET delete_flag=1, update_time=#{now} "
         + "WHERE poll_id=#{pollId} AND user_id=#{userId} AND delete_flag=0 AND option_id<>#{selectedOptionId}")
 int removeOtherActiveVotes(@Param("pollId") Long pollId, @Param("userId") Long userId,
