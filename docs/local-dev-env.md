...
```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml --profile optional up -d
```
...
```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml down
```

停止容器并删除本项目数据卷：

```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml down -v
```
...
```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml ps
```

查看日志：

```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml logs -f
```

只看某个服务日志：

```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml logs -f playmate-mysql
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml logs -f playmate-minio
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml logs -f playmate-redis
```
...
密码见 `deploy/.env.example`。这是本地开发示例密码，不要用于正式环境。
...
```bash
docker compose --env-file deploy/.env.example -f deploy/docker-compose.local.yml --profile optional up -d playmate-redis
```
...
如果端口被占用，可以修改 `deploy/.env.example` 中的宿主机端口，例如：
