...
| 表 | 用途 | 关键字段与索引 |
| --- | --- | --- |
| `t_activity_itinerary` | 活动最终行程 | 稳定 `title` 与结构化执行字段分离：交通方式、出发地、目的地、路线、用餐类型、餐厅、活动内容、地点和地址；按活动日期时间、待决定状态、创建人和来源投票索引。 |
...
- `decision_scope` 是结果应用的持久化字段白名单；选项 `result_payload` 只能包含白名单字段，后端预览和正式应用均再次校验。
- 稳定的行程名称只会在 `decision_scope` 明确包含 `title` 时修改。交通方式投票不会覆盖行程名称、日期时间、出发地或目的地。
