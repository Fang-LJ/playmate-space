...
MISSING_FULL_SELECTION=$(api -o /dev/null -w '%{http_code}' -X POST "$BASE_URL/api/activities/$ACTIVITY_ID/polls/$FULL_POLL_ID/close" -H "Authorization: Bearer $TA")
[[ "$MISSING_FULL_SELECTION" == 400 ]] || fail "FULL_PLAN 结束前必须选择方案"
...
FULL_APPLIED=$(api -X POST "$BASE_URL/api/activities/$ACTIVITY_ID/polls/$FULL_POLL_ID/close" -H "Authorization: Bearer $TA" -H 'Content-Type: application/json' -d "{\"optionId\":$FULL_WINNER}")
...
[[ $(printf '%s' "$FULL_APPLIED" | jq -r '.data.resultApplyStatus') == APPLIED
  ...
  && $(printf '%s' "$FULL_DETAIL" | jq -r '.data.itinerary.description') == null ]] || fail "完整方案结束并应用结果错误"
pass "完整方案必须选择后结束，并直接应用标题、时间和执行字段"
...
KEEP_APPLIED=$(api -X POST "$BASE_URL/api/activities/$ACTIVITY_ID/polls/$KEEP_POLL_ID/close" -H "Authorization: Bearer $TA" -H 'Content-Type: application/json' -d "{\"optionId\":$KEEP_OPTION}")
...
pass "保持当前方案可直接结束并保存空 changedFields"
