...
当前目录是「玩伴空间」微信小程序原生工程，使用 WXML / WXSS / JS 开发。V1.0 暂不提供照片墙，V1.0.2 暂不提供投票；恢复说明见 [功能下线记录](../docs/v1.0-disabled-features.md)。
...
V1.0.2 提供行程、待办、账本与 AA；照片墙和投票暂不开放。
...
微信首次创建账号时只会出现一次可跳过的资料提示，不会阻塞登录、活动列表或邀请页回跳。微信资料页中的三项操作彼此独立：
...
第一版费用模块只记录账单、分摊和实时 AA 建议，不记录线下转账是否完成。费用详情页初始加载只请求 dashboard 和账单列表，切换“账单 / 成员 / 结算”不会重复计算。

1. 创建者记账时可选择付款人；普通成员的付款人固定为自己。
2. 平均分摊会把分尾差稳定分配到成员；自定义分摊必须与账单总额一致才能保存。
3. 账单保存后，费用详情会展示每个人的实际付款、应承担、应收/应付以及“谁给谁多少钱”。
4. 编辑账单时会携带 `version`；旧页面再次保存应提示刷新，不能覆盖新数据。
5. 点击“删除账单”后，后端使用 `VOID` 保留历史记录，但该账单不再出现在默认列表，也不参与 AA 计算。

命令行语法检查不能完全替代微信开发者工具。请在工具中至少手动验证一次记账、分类筛选、自定义分摊、编辑冲突和删除后余额刷新。
