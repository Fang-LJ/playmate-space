...
const { ITINERARY_STATUS, dateGroupMeta, formatTimeRange, itinerarySummary, label } = require('../../utils/p1-display');
...
Page({
  ...
  group(items) {
    ...
    return Object.keys(groups).sort().map((date) => ({ ...dateGroupMeta(date, groups[date].length), items: groups[date] }));
  },
