...
Page({
  ...
  voidExpense() { wx.showModal({ title: '删除账单', content: '删除后该账单将不再参与 AA 计算。', confirmColor: '#d94c4c', success: async result => { if (!result.confirm) return; try { await expense.voidExpense(this.data.activityId, this.data.expenseId, { expectedVersion: this.data.detail.version, reason: '' }); wx.showToast({ title: '已删除', icon: 'success' }); wx.navigateBack(); } catch (error) { wx.showToast({ title: error.message || '删除失败', icon: 'none' }); if ((error.message || '').includes('刷新')) this.load(); } } }); },
