...
Page({
  ...
  onLoad(options) { this.expenseContext = createExpenseContext(options); this.setData({ activityId: options.activityId || '', expenseId: options.expenseId || '' }); },
  onShow() { if (this.data.expenseId) this.load(); },
  async load() {
    this.setData({ loading: true });
    try {
      const [detail, currentUser] = await Promise.all([
        this.expenseContext.getExpense(this.data.expenseId),
        this.expenseContext.viewer()
      ]);
      this.setData({ detail: this.normalizeDetail(detail, currentUser.userId, currentUser.accountUserId || currentUser.userId), errorMessage: '' });
    } catch (error) {
  ...
  normalizeDetail(detail, currentUserId, accountUserId = currentUserId) {
    ...
    return {
      ...
      creatorDisplayName: String(detail.createdBy) === String(accountUserId) ? '我' : detail.creatorNickname,
      splitModeText: detail.splitMode === 'CUSTOM' ? '自定义分摊' : detail.splitMode === 'PROPORTIONAL' ? '按比例分摊' : '平均分摊',
      shareCount: shares.length
    };
  },
  edit() { wx.navigateTo({ url: `/pages/expense-edit/index?${this.expenseContext.query}&expenseId=${this.data.expenseId}` }); },
  voidExpense() { wx.showModal({ title: '作废账单', content: '作废后该账单将不再参与 AA 计算。', confirmColor: '#d94c4c', success: async result => { if (!result.confirm) return; try { await this.expenseContext.voidExpense(this.data.expenseId, { expectedVersion: this.data.detail.version, reason: '' }); wx.showToast({ title: '已作废', icon: 'success' }); wx.navigateBack(); } catch (error) { wx.showToast({ title: error.message || '作废失败', icon: 'none' }); if ((error.message || '').includes('刷新')) this.load(); } } }); },
